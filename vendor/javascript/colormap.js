...
var e = a;
t = createColormap;
function createColormap(r) {
  var a, t, n, h, l, i, p, u, f, s;
  r || (r = {});
  u = (r.nshades || 72) - 1;
  p = r.format || "hex";
  i = r.colormap;
  ...
  if ("string" === typeof i) {
    i = i.toLowerCase();
    if (!o[i]) throw Error(i + " not a supported colorscale");
    l = o[i];
  } else {
    if (!Array.isArray(i)) throw Error("unsupported colormap option", i);
    l = i.slice();
  }
  if (l.length > u + 1)
    throw new Error(
      i + " map requires nshades to be at least size " + l.length
    );
  f = Array.isArray(r.alpha)
    ? 2 !== r.alpha.length
      ? [1, 1]
      : r.alpha.slice()
    ...
    : [1, 1];
  a = l.map(function (r) {
    return Math.round(r.index * u);
  });
  f[0] = Math.min(Math.max(f[0], 0), 1);
  f[1] = Math.min(Math.max(f[1], 0), 1);
  var g = l.map(function (r, a) {
    var t = l[a].index;
    var o = l[a].rgb.slice();
    if (4 === o.length && o[3] >= 0 && o[3] <= 1) return o;
    o[3] = f[0] + (f[1] - f[0]) * t;
    return o;
  });
  var m = [];
  for (s = 0; s < a.length - 1; ++s) {
    ...
    for (var c = 0; c < h; c++) {
      var b = c / h;
      m.push([
        Math.round(e(t[0], n[0], b)),
        Math.round(e(t[1], n[1], b)),
        Math.round(e(t[2], n[2], b)),
        e(t[3], n[3], b),
      ]);
    }
  }
  m.push(l[l.length - 1].rgb.concat(f[1]));
  "hex" === p
    ? (m = m.map(rgb2hex))
    : "rgbaString" === p
    ? (m = m.map(rgbaStr))
    : "float" === p && (m = m.map(rgb2float));
  return m;
}
function rgb2float(r) {
